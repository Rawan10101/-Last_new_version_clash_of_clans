...
Game::Game(QWidget *parent) : QWidget(parent)
{
    ...
    // moneybar->setPos(10,60);
    // scene->addItem(moneybar);
    // moneybar->hide();
    ...
    // moneyRect = new QGraphicsRectItem;
    // moneyRect->setRect(0,0,40,20);
    // moneyRect->setOpacity(70);
    // QBrush grayBrush(Qt::gray);
    // moneyRect->setBrush(grayBrush);
...
void Game::startLevel()
{
    ...
    // QGraphicsProxyWidget* moneyL = scene->addWidget(moneyLabel);
    ...
    // scene->addItem(moneyImage);
    // moneyImage->setPos(timerRect.x(), timerRect.y() + timerRect.height() + 10);
    // // scene->addItem(moneyLabel);
    // moneyLabel->setGeometry(moneyImage->x() + pixmap1.width(), moneyImage->y(), 60, 20);

    // moneyLabel->setGeometry(scene->width() - 100, 70, 90, 40);
    // moneyImage->setPos(scene->width() - 60, 60);
    // scene->addItem(moneyImage);
    // moneyL->stackBefore(moneyImage);
...
QVector<QPair<int, int>> Game::findshortestPath(QPair<int,int> troopPosition) //returns the next i,j troop should go to
{
        ...
        for (int y = 0; y < tempMap.size(); y++)
            ...
            {
                if (tempMap[y][x] == 3 && scene->items(QPointF(y * 50, x * 50)).isEmpty()) //if there is a fence there and it is destroyed, add 0 instead
                    tempMap[y][x] = 0;
            }
    ...
    while (!queue.empty()) //BFS
    {
        ...
        QVector<QPair<int, int>> adjList =
            { {i + 1, j}, {i - 1, j}, {i, j + 1}, {i, j - 1} }; //adjacent cells are the ones down, up, right, and left
        ...
        for (const auto& adjacent: adjList)
        {
            ...
            if (adjI < tempMap.size() && adjI >= 0 && adjJ < tempMap[0].size() && adjJ >= 0) //to avoid going out of bounds of the vector dimensions
                    // if (!visited[adjI][adjJ]) //to make sure it has not been visited
            {
